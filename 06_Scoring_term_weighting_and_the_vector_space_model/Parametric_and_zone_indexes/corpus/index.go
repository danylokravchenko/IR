...
func (title *TitleIndex) BuildIndexFromTokens(tokens []Token) {

	title.wg.Add(len(tokens))

	for _, t := range tokens {
		go title.createIndexFromToken(t)
	}
...
func (title *TitleIndex) createIndexFromToken(token Token) {

	title.mutex.Lock()

	if index, ok := title.Get(token.Term); !ok {
		...
		title.Put(token.Term, Index{ Docs{docs}, 1})
	} else {
