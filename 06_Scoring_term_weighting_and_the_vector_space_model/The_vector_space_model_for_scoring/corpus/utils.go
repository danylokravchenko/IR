...
func (corpus *Corpus) CountInverseDocumentsFrequency() {
	...
	corpus.Each(func(key, value interface{}) {
		...
		index.Docs.Each(func(key, value interface{}) {
			doc := value.(Doc)
			doc.InverseDocumentFrequency = CountInverseDocumentFrequency(corpus.DocsNum,doc.Frequency)
...
func CountInverseDocumentFrequency(N int, frequency int) float32 {
	idf := float32(math.Log(float64(N)/float64(frequency)))
	if idf <= 0 {
		return 0.001
	} else {
		return idf
...
func (corpus *Corpus) CountNormalizedDocumentsFrequency() {

	corpus.Documents.Each(func(key, value interface{}) {
		docs := value.(DocumentIndex)
		docs.Each(func(key, value interface{}) {
			docs.Put(key, float32(value.(int))/float32(docs.Size()))
		})
	})
